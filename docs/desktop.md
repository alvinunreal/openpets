...
- **Pet-window media CSP**: both HTML documents generated by
  `createBuiltInPetRender` and `createInstalledPetRender` allow imported audio
  data URLs through the sole media exemption, `media-src data:`. They do not
  allow file or network media sources.
