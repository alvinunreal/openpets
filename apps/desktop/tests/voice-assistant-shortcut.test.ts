...
result = false;
assert.equal(manager.configure("CommandOrControl+Alt+Space").status, "conflict");
assert.deepEqual(unregistrations, [DEFAULT_VOICE_ASSISTANT_SHORTCUT], "replacement unregisters the exact previous accelerator");
result = new Error("global shortcut unavailable");
assert.equal(manager.configure("CommandOrControl+Shift+T").status, "unavailable");
assert.deepEqual(unregistrations, [DEFAULT_VOICE_ASSISTANT_SHORTCUT], "failed replacement does not unregister an accelerator twice");
assert.equal(manager.configure("bad shortcut").status, "invalid");
manager.shutdown();
assert.deepEqual(unregistrations, [DEFAULT_VOICE_ASSISTANT_SHORTCUT]);
result = true;
const shutdownManager = new VoiceAssistantShortcutManager(registry, () => {});
shutdownManager.configure("CommandOrControl+Shift+T");
shutdownManager.shutdown();
assert.deepEqual(unregistrations, [DEFAULT_VOICE_ASSISTANT_SHORTCUT, "CommandOrControl+Shift+T"], "shutdown unregisters the exact active accelerator");
...
assert.equal(retainedManager.configure("CommandOrControl+Shift+Y").status, "unavailable");
...
console.log("Voice assistant shortcut lifecycle verified.");
